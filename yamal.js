...
function saveCanvasFrame(canvas, frameNumber) {
...
pane.addInput(params, "trackingMaxDistance", { min: 20, max: 200, step: 1 });
...
function getDirectionLabel(vx, vy) {
  const threshold = 0.8;
  ...
  if (vx > threshold) horizontal = "R";
  else if (vx < -threshold) horizontal = "L";

  if (vy > threshold) vertical = "D";
  else if (vy < -threshold) vertical = "U";
...
function updateTrackedObjects(blobs) {
  ...
  const smoothing = clamp(params.velocitySmoothing, 0, 0.95);
  ...
  for (const blob of blobs) {
    ...
    for (const track of trackedObjects) {
      ...
      const trackCenter = getBlobCenter(track);
      const d = getDistance(center, trackCenter);
    ...
    if (bestTrack) {
      const prevCenter = getBlobCenter(bestTrack);

      bestTrack.prevX = prevCenter.x;
      bestTrack.prevY = prevCenter.y;

      bestTrack.x = blob.x;
      bestTrack.y = blob.y;
      bestTrack.w = blob.w;
      bestTrack.h = blob.h;
      bestTrack.area = blob.area;
      bestTrack.matched = true;
      bestTrack.missed = 0;
      bestTrack.lastDistance = bestDistance;

      const newCenter = getBlobCenter(bestTrack);
      const rawVx = newCenter.x - bestTrack.prevX;
      const rawVy = newCenter.y - bestTrack.prevY;

      bestTrack.vx = (bestTrack.vx || 0) * smoothing + rawVx * (1 - smoothing);
  ...
  trackedObjects = trackedObjects.filter((track) => track.missed <= maxMissed);

  matchedTracks.sort((a, b) => b.area - a.area);
...
function drawAsciiObjectOutsideBoxes(ctx, motionMask, pixels, blobs, width, height, bass) {
...
function getBlobKey(blob) {
  ...
  return `${idPart}_${kx}_${ky}_${kw}_${kh}`;
}
...
function drawOuterTargetHUD(ctx, blob, i, bass, frame) {
  ...
  ctx.fillText(`TARGET_${String(blob.id || i + 1).padStart(2, "0")}`, panelX + 8, panelY + 13);

  const speed = blob.speed || 0;
  const vx = blob.vx || 0;
  const vy = blob.vy || 0;
  const dir = getDirectionLabel(vx, vy);
  ...
  ctx.fillText(`SPD:${speed.toFixed(1)} DIR:${dir}`, panelX + 8, panelY + 51);
  ctx.fillText(`VX:${vx.toFixed(1)} VY:${vy.toFixed(1)}`, panelX + 8, panelY + 63);
...
function drawDataHUD(ctx, blob, i, bass, frame) {
  ...
  if (!params.outerHud) {
    ...
    ctx.fillStyle = "rgba(0,255,120,0.98)";
    ctx.font = "9px monospace";
    ctx.fillText(`TRACK_${String(blob.id || i + 1).padStart(2, "0")}`, labelX + 6, labelY);
    ctx.fillText(`SIZE:${Math.round(w)}x${Math.round(h)}`, labelX + 6, labelY + 11);
    ctx.fillText(`AREA:${area}`, labelX + 6, labelY + 22);
    ctx.fillText(`SPD:${speed.toFixed(1)}`, labelX + 82, labelY + 22);
    ctx.fillText(`VX:${vx.toFixed(1)}  VY:${vy.toFixed(1)}`, labelX + 6, labelY + 33);
    ctx.fillText(`DIR:${dir}`, labelX + 6, labelY + 44);
  }

  ctx.restore();
}

function drawBoxTag(ctx, blob, label = null) {
  const finalLabel = label || `TRACK_${String(blob.id || 0).padStart(2, "0")}`;
  const tx = blob.x;
  ...
  ctx.fillText(finalLabel, tx + 4, ty + 8);
  ctx.restore();
}

function drawVelocityTag(ctx, blob) {
  const speed = blob.speed || 0;
  const dir = getDirectionLabel(blob.vx || 0, blob.vy || 0);

  const tx = blob.x;
  const ty = blob.y + blob.h + 6;

  ctx.save();
  ctx.fillStyle = "rgba(0,0,0,0.72)";
  ctx.fillRect(tx, ty, 78, 12);

  ctx.strokeStyle = "rgba(0,255,120,0.85)";
  ctx.strokeRect(tx, ty, 78, 12);
...
const sketch = async () => {
  ...
  window.addEventListener("keydown", async (e) => {
    ...
    if (key === "r") {
      ...
      trackedObjects = [];
      nextTrackId = 1;
      boxEffectMemory.clear();
      blobNodeMemory.clear();
      ...
      console.log("PNG RECORD START");
      console.log("Setelah selesai, gabungkan dengan ffmpeg:");
      console.log('ffmpeg -framerate 30 -i frame_%05d.png -i audio/criswar2.mp3 -c:v libx264 -pix_fmt yuv420p -c:a aac -shortest output.mp4');
    }
    ...
    if (key === "s") {
      ...
      console.log("PNG RECORD STOP");
      console.log("Gabungkan PNG + audio dengan command:");
      console.log('ffmpeg -framerate 30 -i frame_%05d.png -i audio/criswar2.mp3 -c:v libx264 -pix_fmt yuv420p -c:a aac -shortest output.mp4');
    }
  });
  ...
  return ({ context, width, height }) => {
    ...
    const trackedBlobs = params.blobTracking
      ? updateTrackedObjects(blobs)
      : [];

    if (params.isolateToTrackedBoxes && trackedBlobs.length) {
    ...
    if (params.isolateToTrackedBoxes && trackedBlobs.length) {
      drawIsolatedTrackedBoxes(context, frameBufferCanvas, trackedBlobs, width, height);
    } else {
    ...
    if (params.thermal && trackedBlobs.length) {
    ...
    if (params.blobTracking) {
      trackedBlobs.forEach((blob, i) => {
        if (params.boxEffects) {
          ...
          drawBoxTag(context, blob, `TRACK_${String(blob.id).padStart(2, "0")}`);
          drawVelocityTag(context, blob);
        }
    ...
    drawRecordingHUD(context, width, height);

    if (isRecordingFrames) {
      saveCanvasFrame(context.canvas, recordedFrameCount);
      recordedFrameCount++;

      if (shouldAutoStop && recordedFrameCount >= settings.fps * maxRecordSeconds) {
        isRecordingFrames = false;
        if (audio) audio.pause();
        if (video) video.pause();

        console.log(`AUTO STOP after ${maxRecordSeconds} seconds`);
        console.log('ffmpeg -framerate 30 -i frame_%05d.png -i audio/criswar2.mp3 -c:v libx264 -pix_fmt yuv420p -c:a aac -shortest output.mp4');
      }
    }
  };
